...
parser.add_argument('--image_dir',type=str,default='no',help='input directory, needs /')
...
idir_name=os.path.basename(os.path.dirname(args.image_dir))
print(idir_name)

data = ImageClassifierDataLoader.from_folder(args.image_dir)


label=list(os.walk(args.image_dir))[0][1]
print(label)
for item in pathlib.Path(args.image_dir).iterdir():
	print(item)
all_image_paths=list(pathlib.Path(args.image_dir).glob('*/*'))
